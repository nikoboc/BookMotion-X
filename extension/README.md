...
- **重複防止（Notion が真実）**: 各行に注釈 ID（Amazon の注釈 id、無ければ `本タイトル|位置|引用先頭40字` の合成キー）を保存。同期開始時に **Notion をクエリして既存の注釈 ID を取得**し、無いものだけ登録する。別 PC・手動削除・拡張再インストールにも強い
- **パイプライン実行**: 本タイトル順に「取得 → 即登録」を流し、次の本を先読み取得（6 冊先まで）しながら登録するので、取得時間が登録の裏に隠れる
