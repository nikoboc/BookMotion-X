...
| **Kindle ログイン** | その PC のブラウザで `read.amazon.co.jp` にログイン済みであること（または `cookies.txt` を用意） |
...
## Cookie について（Windows での注意）

「**ブラウザから自動**」は `browser_cookie3` でブラウザの Cookie を読みます。Windows では
**Chrome / Edge / Brave / Firefox** に対応（Safari は Windows に無いので不可）。

最近の Chrome / Edge は Cookie 暗号化が強化され、自動取得に失敗することがあります。その場合は
拡張機能「**Get cookies.txt LOCALLY**」などで `read.amazon.co.jp` の `cookies.txt` を書き出し、
「**cookies.txt**」を選んで指定してください（こちらは確実に動きます）。
...
.\.venv\Scripts\python.exe ..\app\kindle_notion.py -b edge                    # Edge の Cookie を使う
.\.venv\Scripts\python.exe ..\app\kindle_notion.py -c cookies.txt             # 通常実行
```

| 引数 | 意味 |
|---|---|
| `-c, --cookies-file` | エクスポート済み `cookies.txt` を使う |
| `-b, --browser` | Cookie 取得元ブラウザ: `chrome` / `edge` / `brave` / `firefox` |
